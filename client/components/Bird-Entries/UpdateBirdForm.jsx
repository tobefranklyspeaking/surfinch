...
const UpdateBirdForm = ({ birdEntry }) => {
  ...
  useEffect(() => {
    if (birdEntry) {
      setSpecies(birdEntry.bird);
      setDate(new Date(birdEntry.date));
      setNotes(birdEntry.notes);
      setStreet(birdEntry.street_sighted);
      setCity(birdEntry.city_sighted);
      setSt(birdEntry.state_sighted);
    }
  }, [birdEntry]);
